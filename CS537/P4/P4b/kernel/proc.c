...
growproc(int n)
{
  ...
  proc->sz = sz;
	...
	//Sets p size to parent size if is a parent thread
	for(p = ptable.proc; p < &ptable.proc[NPROC]; p++) {
...
exit(void)
{
  ...
  for(p = ptable.proc; p < &ptable.proc[NPROC]; p++){
		//Checks if is a process or a thread
    if(p->parent == proc && p->pgdir != proc->pgdir){
      p->parent = initproc;
      if(p->state == ZOMBIE)
        wakeup1(initproc);
		...
		if(p->parent == proc && p->pgdir == proc->pgdir){
			//Copy parent pg table for child
...
int join(void** stack) {
	...
	//Scan the page table looking for zombie children
	//i.e. completed children waiting to be exited
	for(;;) {
		havethreads = 0;
